...
describe('API Client', () => {
  ...
  describe('Authentication', () => {
    it('should request OTP successfully', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({ phoneNumber: '+254700000000' }),
        })
      );
    ...
    it('should verify OTP and return token', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({ phoneNumber: '+254700000000', code: '123456' }),
        })
      );
    ...
    it('should handle authentication errors', async () => {
      ...
      await expect(api.verifyOtp('+254700000000', 'wrong-code')).rejects.toThrow('Invalid OTP');
    });
  });
  ...
  describe('Member Endpoints', () => {
    ...
    it('should update member profile', async () => {
      ...
      const { api } = await import('../api');
      api.setToken('test-token');
      const result = await api.updateProfile({ fullName: 'Updated Name', location: 'Nairobi' });

      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/members/profile'),
        expect.objectContaining({
          method: 'PATCH',
          body: JSON.stringify({ fullName: 'Updated Name', location: 'Nairobi' }),
        })
      );
  ...
  describe('Subscription Endpoints', () => {
    it('should fetch subscription packages', async () => {
      const mockPackages = [
        ...
        { tier: 'SILVER', monthlyPrice: 1000, benefits: ['Benefit 1', 'Benefit 2'] },
        { tier: 'GOLD', monthlyPrice: 2000, benefits: ['Benefit 1', 'Benefit 2', 'Benefit 3'] },
      ];
